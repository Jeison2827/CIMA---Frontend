...
const WorkerProjects = () => {
    ...
    const fetchProjects = async () => {
      try {
        const response = await axios.get(
          'http://51.79.68.121:5173/developer/projects/worker/projects',
          {
  ...
  const handleStatusChange = async (projectId, newStatus) => {
    try {
      await axios.put(
        `http://51.79.68.121:5173/developer/projects/${projectId}/status`,
        { status: newStatus },
        { headers: { 'accesstoken': accessToken } }
      );
      fetchProjects();
  ...
  const fetchProjectTasks = async (projectId) => {
    try {
      ...
      const response = await axios.get(
        `http://51.79.68.121:5173/developer/projects/${projectId}/worker/tasks`, // Updated endpoint
        {
          headers: { 'accesstoken': accessToken }
        }
      );
